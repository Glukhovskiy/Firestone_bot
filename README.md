...
- **Guardian** - Автоматическая тренировка и просвещение стражей (раз в 5 минут)
- **Sales** - Автоматическая продажа предметов у экзотического торговца (раз в 5 минут)
...
Настройки продажи предметов в секции `SalesItems`:

```json
"SalesItems": {
  "MidasTouch": true,           // Продавать
  "PouchOfGold": false,         // Не продавать (золото)
  "BucketOfGold": false,        // Не продавать (золото)
  "CrateOfGold": false,         // Не продавать (золото)
  "PileOfGold": false,          // Не продавать (золото)
  "DrumsOfWar": true,           // Продавать
  "DragonArmor": true,          // Продавать
  "GuardiansRune": true,        // Продавать
  "TotemOfAgony": true,         // Продавать
  "TotemOfAnnihilation": true   // Продавать
}
```

**Принцип**: сохраняем золото, продаем артефакты.

### Настройки стражей

Настройки тренировки стражей в секции `GuardianSettings`:
...
**Описание параметров:**
- `Angel`, `Dragon`, `Fenix`, `Jinn` - выбор конкретных стражей для тренировки
- `Enlightenment` - включение/отключение просвещения стражей

**Особенности:**
- Можно выбрать несколько стражей одновременно
- Проверка уведомлений о тренировке происходит раз в 5 минут
- Просвещение можно отключить независимо от тренировки
...
16. **Guardian** - Тренировка и просвещение стражей
17. **Sales** - Продажа предметов у экзотического торговца
18. **Close Windows** - Закрытие зависших окон
...
- **Guardian**: Проверка уведомлений о тренировке стражей раз в 5 минут
- **Oracle's Gift**: Оптимизировано для выполнения раз в 5 минут
